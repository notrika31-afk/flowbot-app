...
import { useRouter, useSearchParams } from 'next/navigation'; // הוספנו את useSearchParams
...
export default function RegisterPage() {
  ...
  // בדיקה: האם יש לאן לחזור? (למשל /builder), אם לא - ברירת מחדל לדשבורד
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // רענון הנתונים בשרת (כדי שה-Header יתעדכן)
        router.refresh();
        
        // העברה ליעד החכם (למשל Builder) במקום סתם לדשבורד
        setTimeout(() => {
          router.push(callbackUrl);
        }, 500);
      } else {
        setError(data.error || 'שגיאה ביצירת החשבון. נסה שוב.');
      }
    } catch (err) {
      ...
      >
        <div className="bg-white border-[3px] border-black rounded-[24px] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8">
          ...
          <div className="text-center mb-8">
            <div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center mx-auto mb-4 shadow-md relative overflow-hidden group">
               <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
               <Sparkles className="w-6 h-6 text-white relative z-10" />
            </div>
            <h1 className="text-2xl font-black text-black">יצירת חשבון חדש</h1>
            <p className="text-gray-500 text-sm mt-2 font-medium">הצטרף ל-FlowBot והתחל לבנות</p>
          </div>
          ...
          <div className="mt-6 text-center pt-6 border-t border-gray-100">
             {/* שומרים על ה-CallbackUrl גם אם עוברים ללוגין */}
             <Link 
             ...
             >
               כבר יש לך חשבון? <span className="font-bold underline decoration-2 decoration-transparent group-hover:decoration-black underline-offset-4 transition-all">התחבר כאן</span>
             </Link>
          </div>
        </div>
      </motion.div>
